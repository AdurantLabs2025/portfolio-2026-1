...
import { CardRow, ShimmerText, MergedCard } from './helpers';

const STIFFNESS = 0.032;
const DAMPING = 0.88;
const PROXIMITY_PX = 180;

function CardDragExperiment() {
    ...
    if (!s.dragging && dragRef.current) {
      if (s.docked) {
        ...
        if (s.dockTargetY !== 0 && staticRef.current) {
          ...
          targetY = (staticRect.top + 92) - restRect.top;
        }
  ...
  const move2=useCallback((e)=>{
    ...
    if(o2.current){
      ...
      o2.current.style.transform=`scale(${scale}) rotate(${d}deg)`;
    }
    s.x=tx-s.restLeft; s.y=ty-s.restTop;
  },[]);
  ...
  const renderCard = (icon, title, sub1, sub2, outerRef, v, isHov, setHov, onDown, expand) => {
    const { prox, dragging, docked } = v;
    const extraBot = expand * 103;
    ...
    return (
      <div style={{ width: 442, overflow: "visible", position: "relative" }}>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:16 }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ marginBottom:7 }}>
                      <ShimmerText style={{ fontSize:21,fontWeight:500,letterSpacing:"-0.03em",lineHeight:1.2 }}>{title}</ShimmerText>
                    </div>
                    <div style={{ display:"flex",alignItems:"center",gap:14,fontSize:16,fontWeight:400 }}>
                      <ShimmerText style={{}}>{sub1}</ShimmerText>
                      <span style={{ width:1,height:16,background:"#d1d5db" }}/>
                      <ShimmerText style={{}}>{sub2}</ShimmerText>
                    </div>
                  </div>
                </div>
              ) : (
                <CardRow icon={icon} title={title} sub1={sub1} sub2={sub2} />
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <div ref={containerRef} style={{ width:"100%",height:"100%",background:"#e2e5ed",overflow:"hidden",fontFamily:"'Inter',-apple-system,sans-serif",userSelect:"none",position:"relative" }}>
      <div style={{ position:"absolute",bottom:120,left:"50%",transform:"translateX(-50%)",color:"#9aa0b4",fontSize:16,whiteSpace:"nowrap",opacity:anyActive||anyDocked?0:1,transition:"opacity 0.5s ease",pointerEvents:"none" }}>
      ...
      <button onClick={()=>{
        ...
        style={{ position:"absolute",bottom:120,left:"50%",transform:`translateX(-50%) translateY(${(anyDocked||mergePhase)?0:12}px)`,opacity:(anyDocked||mergePhase)?1:0,pointerEvents:(anyDocked||mergePhase)?"auto":"none",transition:"opacity 0.35s ease,transform 0.35s ease",background:"white",border:"none",borderRadius:8,padding:"10px 22px",fontSize:15,fontWeight:500,color:resetHov?"#111":"#333",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.08)",fontFamily:"'Inter',-apple-system,sans-serif",display:"flex",alignItems:"center" }}>
      ...
      <div style={{ position:"absolute",top:0,left:0,right:0,bottom:0,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none",zIndex:50 }}>
        <div style={{ pointerEvents:"auto",position:"relative",height:344 }}>
        ...
        <div style={{
          ...
          display: "flex", flexDirection: "column", gap: 160, position: "relative",
        }}>
          <div ref={card1Wrap} style={{ position: "relative", minHeight: 92, zIndex: (v1.dragging || v1.docked) ? 100 : 1 }}>
            {renderCard(mkImg(WORKOUT_IMG),"High-intensity Workout","Wednesday","30 min",o1,v1,hover1,setHover1,down1,expand1)}
            ...
            {(() => {
              ...
              return <div style={{
                ...
                <svg width="442" height="92" style={{ position:"absolute", top:0, left:0 }}>
                  <rect x="0.5" y="0.5" width="441" height="91" rx="22" ry="22"
          ...
          <div style={{ position: "relative", minHeight: 92, marginTop: expand1 ? -263 : 0, transition: "margin-top 0.35s cubic-bezier(0.34, 1.56, 0.64, 1)", zIndex: v2.dragging ? 100 : 1 }}>
            {renderCard(mkImg(MUSIC_IMG),"Your Library","325 Songs","84 Artists",o2,v2,hover2,setHover2,down2,expand2)}
        ...
        {isRevealed && (
          <div ref={mergedRef} style={{
            ...
            <MergedCard visible={true} onPointerDown={mergedDown} />
          </div>
        )}
      </div>
      </div>
...
export default CardDragExperiment;
